...
class AliveBot:
    ...
    def __init__(self, app):
        ...
        self.clients = {}
        for c in service:
            if 'public' in c and 'disabled' not in c:
    ...
    async def check_client(self, chat_id):
        ...
        if 'public' not in client or client['disabled']:
    ...
    async def check_client_job(self, context: CallbackContext):
        await self.check_client(context.job.data)
...
Registered with public channel: {public_chat_id}
Alive monitoring is currently disabled.
""", 2)
                else:
...
""", 2)
                ...
                del client['public']
                text = helpers.escape_markdown(f"Deregistered.", 2)
            elif action == 'enable':
                del client['disabled']
                if not client['public']:
                    text = helpers.escape_markdown(f"Enabled, but no public broadcast channel registered.", 2)
                else:
                    self.new_client(chat_id)
                    public_chat_id = int(client['public'])
                    text = helpers.escape_markdown(f"""
Enabled with public channel: {public_chat_id}
See you again in {round(timedelta(seconds=self.interval).total_seconds()/60.0/60.0)} hours!
""", 2)
            elif action == 'disable':
                client['disabled'] = True
                text = helpers.escape_markdown(f"Disabled.", 2)
            elif action == 'custom':
                if len(query) < 2:
                    del client['custom']
                else:
...
`/alive cancel`
Cancel check in monitoring, and deregister public broadcast channel
`/alive disable`
Disable check in monitoring, but don't deregister public broadcast channel
`/alive enable`
Re\\-enable check in monitoring
